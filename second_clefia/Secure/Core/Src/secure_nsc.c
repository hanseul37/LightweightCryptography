...
CMSE_NS_ENTRY void encrypt(uint8_t* plain, uint8_t* cipher, uint8_t* iv, uint8_t* key){
	...
	uint8_t cipher_temp[STRING_LENGTH];
	uint8_t key_temp[16];
	uint8_t iv_temp[16];
	...
	clefia_cbc_128_enc(plain_temp, cipher_temp, STRING_LENGTH, (uint32_t*)iv_temp, (uint32_t*)key_temp);

	memcpy(cipher, cipher_temp, STRING_LENGTH);
	return;
}
...
CMSE_NS_ENTRY void decrypt(uint8_t* plain, uint8_t* cipher, uint8_t* iv, uint8_t* key){
	...
	uint8_t key_temp[16];
	uint8_t iv_temp[16];
	memcpy(cipher_temp, cipher, STRING_LENGTH);
	memcpy(key_temp, key, 16);
	memcpy(iv_temp, iv, 16);

	clefia_cbc_128_dec(plain_temp, cipher_temp, STRING_LENGTH, (uint32_t*)iv_temp, (uint32_t*)key_temp);
